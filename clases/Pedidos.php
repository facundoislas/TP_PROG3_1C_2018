...
class pedido
{
	...
	{
			$objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
			$consulta =$objetoAccesoDato->RetornarConsulta("SELECT p.`idPedido`, p.`nroMesa`, p.`cliente`, p.`foto`, p.`importe`,p.`fecha`, p.`detalleBar`, p.`tiempo_estimado_bar`, p.`tiempo_final_bar`, p.`estadoBar`, p.`idEmpladoBar`, p.`detalleCer`, p.`tiempo_estimado_cer`, p.`tiempo_final_cer`, p.`estadoCer`, p.`idEmpladoCer`, p.`detalleCoc`, p.`tiempo_estimado_coc`,p.`tiempo_final_coc`, p.`estadoCoc`, p.`idEmpladoCoc` FROM `pedidos` AS p  WHERE p.`idPedido`= '$idPedido'");
			$consulta->execute();
    ...
    public static function cambiarTodosEstadoSector($auxID,$auxEstado)
    {
    ...
    public static function TraerCantidadOperacionesSector($tipo)
    {
        $objetoAccesoDatos = AccesoDatos::dameUnObjetoAcceso();
       
            $consulta = $objetoAccesoDatos->RetornarConsulta("SELECT count(*) cant FROM pedidos WHERE $tipo = 'Finalizado' ");
    ...
    public static function TraerUsosMesas()
    {
        $objetoAccesoDatos = AccesoDatos::dameUnObjetoAcceso();
        
            $consulta = $objetoAccesoDatos->RetornarConsulta("SELECT `nroMesa`,COUNT(`nroMesa`) cant FROM `pedidos`  GROUP BY `nroMesa` ORDER BY cant DESC");
